...
- `NamedPipe` and `TcpSocket` are the current built-in transports used by the cross-process runtime
- `TcpSocket` requires an explicit `host:port` endpoint; Lifecycle does not infer TCP ports from service identity
- both built-ins use the `extend0-jsonrpc-ndjson` protocol descriptor and shared handshake validation
