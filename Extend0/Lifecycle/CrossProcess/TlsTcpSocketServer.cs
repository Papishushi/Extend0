...
{
    /// <summary>
    /// JSON-RPC NDJSON host over TCP protected by TLS.
    /// </summary>
    internal sealed class TlsTcpSocketServer : ICrossProcessServerHost
    {
        ...
        private readonly CrossProcessTlsOptions _tls;
        private readonly object _impl;
        private readonly ILogger<TlsTcpSocketServer> _logger;
        private readonly CancellationTokenSource _cts;
        private readonly TcpListener _listener;
        private readonly Task _loopTask;
        private bool _disposed;

        public TlsTcpSocketServer(
            string endpointName,
            object impl,
            ILogger<TlsTcpSocketServer> logger,
            ...
            CrossProcessTlsOptions? tls = null)
        {
            ...
            _protocol = protocol ?? TlsTcpSocketTransportProtocol.Descriptor;
            _authentication = authentication ?? CrossProcessAuthenticationOptions.None;
            _tls = tls ?? throw new ArgumentException("TLS TCP server hosting requires TLS options with a server certificate.", nameof(tls));
            if (_tls.ServerCertificate is null)
                throw new ArgumentException("TLS TCP server hosting requires a server certificate.", nameof(tls));
            ...
            try
            {
                while (!token.IsCancellationRequested)
                {
                    ...
                    HandleClientAsync(client, methodsByName, token)
                        .Forget(_logger,
                            onExceptionMessage: "TlsTcpSocketServer: client handler crashed",
                            onExceptionAction: ex => _logger.LogCritical(ex, "TLS TCP RPC handler fatal"),
        ...
        private async Task HandleClientAsync(TcpClient client, IReadOnlyDictionary<string, MethodInfo[]> methodsByName, CancellationToken token)
        {
            ...
            try
            {
                ...
                tlsStream = new SslStream(
                    ...
                    _tls.RemoteCertificateValidationCallback);

                await AuthenticateTlsAsync(tlsStream, token).ConfigureAwait(false);

                reader = new StreamReader(tlsStream, Encoding.UTF8, detectEncodingFromByteOrderMarks: false, leaveOpen: true);
                writer = new StreamWriter(tlsStream, new UTF8Encoding(encoderShouldEmitUTF8Identifier: false), bufferSize: 1024, leaveOpen: true)
                {
                    AutoFlush = true
                };
            ...
            {
                _logger.LogWarning(ex, "TLS authentication failed for endpoint {Endpoint}.", _endpoint);
            }
            catch (IOException)
            {
            }
            ...
            finally
            {
                ...
                try { tlsStream?.Dispose(); } catch (ObjectDisposedException) { }
                try { networkStream?.Dispose(); } catch (ObjectDisposedException) { }
                try { client.Dispose(); } catch (ObjectDisposedException) { }
            }
        }

        private Task AuthenticateTlsAsync(SslStream tlsStream, CancellationToken token)
        {
            var options = new SslServerAuthenticationOptions
            {
                ...
                CertificateRevocationCheckMode = _tls.CheckCertificateRevocation
                    ? X509RevocationMode.Online
                    : X509RevocationMode.NoCheck
            };

            return tlsStream.AuthenticateAsServerAsync(options, token);
        }
            ...
            CancellationToken token)
        {
            ...
            {
                _logger.LogWarning("Rejected cross-process client authentication for TLS TCP endpoint {Endpoint}: authentication mode {AuthenticationMode} is not supported by this transport.", _endpoint, _authentication.Mode);
            ...
            {
                _logger.LogWarning("Rejected cross-process client authentication for TLS TCP endpoint {Endpoint}: {Reason}", _endpoint, error);
                await writer.WriteLineAsync(CrossProcessHandshake.BuildAuthenticationErrorLine()).ConfigureAwait(false);
                return false;
            }
